...
import { usePratoEdicaoViewModel } from '../hooks/usePratosViewModel';
import type { RestauranteStackParamList } from '../navigation/types';
import type { CategoriaMenu } from '../types/api';
import { resolveMediaUrl } from '../utils/imageUrl';
import { palette } from '../theme/colors';

type Props = NativeStackScreenProps<RestauranteStackParamList, 'PratoEditar'>;
...
export function PratoEditarScreen({ navigation, route }: Props): React.JSX.Element {
  ...
  const vm = usePratoEdicaoViewModel(rid, prato);
  ...
  const pickImage = async () => {
    ...
    if (!res.canceled && res.assets[0]) {
      vm.setFoto(res.assets[0].uri, res.assets[0].mimeType ?? 'image/jpeg');
    }
  };

  const fotoAtual = vm.fotoPreviewUri ? resolveMediaUrl(vm.fotoPreviewUri) ?? vm.fotoPreviewUri : null;
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: bgPage }]} edges={['top']}>
      <View style={[styles.appBar, { borderBottomColor: border }]}>
        ...
        <Text style={[styles.appTitle, { color: text }]}>Editar Prato</Text>
        <View style={{ width: 48 }} />
      </View>
      ...
      >
        <View style={styles.uploadBlock}>
          <Pressable onPress={pickImage}>
            {fotoAtual ? (
              <Image source={{ uri: fotoAtual }} style={styles.photoBox} />
            ) : (
          ...
          <Text style={[styles.uploadSub, { color: sub }]}>
            Troque a foto se quiser atualizar a imagem
          </Text>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.tabs}>
          {CATEGORIAS.map((c) => {
            ...
            return (
              ...
              >
                <Text style={[styles.chipText, active ? { color: palette.white } : { color: text }]}>
                  {c.label}
                </Text>
              </Pressable>
            );
